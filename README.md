...
> Theme1
- 타입을 바라보는 관점
  1. 개념 관점 : 공통의 특징을 공유하는 대상들의 분류
  2. 동일한 오퍼레이션을 적용할 수 있는 인스턴스들의 집합
- 객체지향 프로그래밍 관점에서 객체의 퍼블릭 인터페이스(수신할 수 있는 메시지)가 객체의 타입을 결정
- 여기서 중요한 점! 객체에게 중요한건 속성이 아닌 행동이다! 따라서 내부 상태가 아닌 퍼블릭 인터페이스가 객체의 타입을 결정한다는 것이다.
- 다시 한 번 언급되지만 상속은 타입 계층 구현을 위해 사용되어야 한다
  ▶︎ 조건 1 : 상속 관계가 is-a 관계를 모델링해야 한다  
  ▶︎ 조건 2 : 클라이언트 입장에서 부모 클래스의 타입으로 자식 클래스를 사용해도 무방해야 한다   
  

- 조건 1 : 자식클래스는 는 부모클래스 이다. 라고 말해도 이상하지 않음(이름 연관성)
- 조건 2 : 클라이언트 입장에서 부모클래스와 자식클래스의 차이점을 모름(행동 호환성) -> 더 중요함
  이때, 행동 호환성을 판단하는 기준은 클라이언트의 관점이다

<span style = 'background-color: #fff5b1'>Ex) 펭귄은 새이다. 일반적인 행동 호환성을 만족하기 때문이다. 하지만 클라이언트가 모든 새는 날 수 있다고 가정하고 있다면 펭귄은 새가 아니게 된다. 따라서 펭귄은 새를 상속해서는 안 된다
- 클라이언트 들은 서로 독립적이어야 한다. 따라서 인터페이스를 클라이언트의 기대에 따라 분리해야 한다. 이를 ‘인터페이스 분리 원칙(ISP)’라고 한다</span>

>Theme2
- 서브클래싱 : 코드 재사용을 목적으로 상속을 사용하는 경우 = 구현 상속, 클래스 상속
- 서브타이핑 : 타입 계층을 구성하기 위해 상속을 사용하는 경우 = 인터페이스 상속
- 행동 호환성의 여부가 서브타이핑에서는 매우 중요하다

<span style = 'background-color: #fff5b1'> Ex) 일반적으로 ‘정사각형은 직사각형이다’. 하지만 reX, reY 함수가 직사각형에 정의 되어 가로 세로 너비를 지정할 수 있고, 정사각형은 직사각형을 상속해 reX,Y를 오버라이딩해서 각 함수에서 입력받은 값을 토대로 입력받은 부분 외의 길이도 통일하게 한다고 하자. 이렇게 되면 클라이언트가 직사각형에 기대한 작업이 정사각형에서는 제대로 작동하지 않을 수 있다. 이런 경우 서브타이핑이 아니다</span>

📍리스코프 치환 원칙 (조금 설명이 길어서 짧게 요약할 수 있게 공부하기)  
📍계약에 의한 설계(조금 까다롭지만 흥미로운 주제인거 같다 마지막에 더 살펴보기)


# Chapter 14

> Theme1
- 일관성을 고려하지 않은 설계 : 구체적으로는 비슷한 기능을 통일되지 않은 방식으로 구현하는 경우  -> 기능이 추가됨에 따라 복잡성이 더 올라간다
- 일관성을 고려한 계발을 위해서는 풍부한 계발경험이 가장 도움이 된다 -> 하지만 단기간에 그런 능력을 키우기는 힘들다. 따라서 디자인 패턴을 학습하고 적용해보는 것이다
- 가장 기본적인 지침
  1. 변하는 개념을 변하지 않는 개념으로부터 분리하라 -> 개별적인 객체로 분리하라
  2. 변하는 개념을 캡슐화하라 -> 객체의 타입 계층을 형성하라(인터페이스 및 구현으로 나눔)
- 다형성이란 조건 로직(if~else, switch -> 조건을 추가하면 코드를 수정해야함)을 객체 사이의 이동으로 바꿀 수 있도록 하는 설계 기법이다
> 객체지향적인 코드는 조건을 판단하지 않는다. 단지 다음 객체로 이동할 뿐이다
- 캡슐화는 단순히 ‘데이터 은닉’을 의미하지 않는다. 소프트웨어 안에서 변할 수 있는 모든 ‘개념’을 숨기는 것이다
- 캡슐화 : 데이터 캡슐화(private), 메서드 캡슐화(protected), 객체 캡슐화(합성), 서브타입 캡슐화(다형성) -> 구분하면서 사용해보기
- 일관성 있는 협력은 개발자에게 확장 포인트를 강제하기 때문에 정해진 구조를 우회하기 어렵게 만든다
- 코드를 한 번 이해하면 다른 코드를 이해하는데 직접적으로 사용할 수 있다


# Chapter 15 ⭐⭐⭐
- 디자인 패턴, 프레임워크 : 의존성 역전, 제어의 역전 이해 -> 스프링 공부하면서 곱씹어 보기
>Theme1
- 디자인 패턴 : 설계를 재사용 하는것
- 프레임워크 : 설계와 코드를 함께 재사용 하는 것
- 프레임워크는 일관성 있는 협력을 제공하는 확장 가능한 코드라고 할 수 있다
> 둘 모두 협력을 일관성 있게 만들기 위한 방법이다
- 패턴은 겸험의 산물이다
- 디자인 패턴의 구성요소는 클래스와 메서드가 아니라 역할과 책임이다
> 디자인 패턴을 공부할때 디자인 패턴이 어떤 변경을 캡슐화하는지를 이해하면서 공부해야겠다!
>> <span style = 'background-color: #fff5b1'> 망치를 들면 모든 것이 못으로 보인다 </span>
